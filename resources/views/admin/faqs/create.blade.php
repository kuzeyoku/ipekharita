...
@include('admin.partials.page_header', [
    'title' => 'Yeni SSS Tanımla',
    'subtitle' => 'Hizmetler, Projeler veya Genel sayfalara yeni soru ve cevap bağlayın.',
    'icon' => 'fa-plus',
            ...
            <div class="admin-card-glass p-4 mb-4">
                ...
                <div class="mb-3">
                    ...
                    <select name="module_type" id="moduleTypeSelect" class="form-select admin-input-light @error('module_type') is-invalid @enderror" required>
                        <option value="general" {{ old('module_type') == 'general' ? 'selected' : '' }}>Genel SSS (Tüm Sayfalar)</option>
                        <option value="service" {{ old('module_type') == 'service' ? 'selected' : '' }}>Hizmetler Modülü</option>
                        <option value="project" {{ old('module_type') == 'project' ? 'selected' : '' }}>Projeler Modülü</option>
                    </select>
                ...
                <div class="mb-3" id="projectSelectorWrapper" style="display: none;">
                    ...
                    <select name="project_id" class="form-select admin-input-light">
                        ...
                        @foreach($projects as $prj)
                            <option value="{{ $prj->id }}" {{ old('project_id') == $prj->id ? 'selected' : '' }}>{{ $prj->title }}</option>
                        @endforeach
                    </select>
            ...
            <div class="admin-card-glass p-4">
                ...
                <div class="d-flex gap-2 pt-2">
                    <button type="submit" class="btn btn-enterprise-admin w-100 py-2">
                        <i class="fa-solid fa-floppy-disk me-1"></i> SSS Kaydet
                    </button>
